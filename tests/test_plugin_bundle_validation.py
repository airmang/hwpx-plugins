...
def test_product_identity_is_the_name_version_and_maturity_authority() -> None:
    ...
    assert components["core"]["currentVersion"] == "5.0.2"
    assert components["core"]["minimumCompatibleVersion"] == "5.0.0"
    assert components["automation"]["currentVersion"] == "6.0.4"
    ...
    assert components["plugin"]["currentVersion"] == "1.0.1"
    assert components["plugin"]["minimumCompatibleVersion"] == "1.0.0"
...
def test_product_identity_validator_supports_the_full_release_lifecycle(
    ...
    if status == "release-approved":
        # The real checkout is release-approved: it already carries the
        # previous public train, the unpublished-candidate row, and the state
        # literal, so the synthesis only needs the status stamp above.
        pass
    else:
        promoted = {
            "pythonHwpx": "5.0.2",

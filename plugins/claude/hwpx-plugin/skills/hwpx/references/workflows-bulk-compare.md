...
- **한계 정직 고지(S-085 실측)**: fit은 "칸에 안 맞는 값"만 고친다. 실측(wild 공개 양식
  전수)에서 양식 채움 실패의 **지배 원인은 fit이 아닌 구조적 요인**이었다 — 채움 시 글자
  겹침, 대형 다중 페이지 양식의 경계 밀림, 표 구조 민감도. 짧은 값(한두 글자)조차 이런
  양식에서 레이아웃을 흔들 수 있으므로, **낯선 양식은 항상 실제 한컴 렌더로 전 페이지
  검토**하고(픽셀 보장 아님), 겹침·페이지 증가가 보이면 `needs_review`로 남긴다.

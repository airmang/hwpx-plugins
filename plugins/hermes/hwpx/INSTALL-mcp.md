...
The `2.2.0 / 7.1.0 / 6.4.0` coordinates below are an unpublished candidate; public installation requires its later release.
...
mcp_servers:
  hwpx:
    command: uvx
    args: ["--with", "python-hwpx[preview]>=6.4.0,<7", "--from", "python-hwpx-automation[mcp,oracle]>=7.1.0,<8", "hwpx-automation-mcp"]
    env:
...
`uvx`는 처음 해석한 버전 조합을 캐시하고 스스로 갱신하지 않습니다. 같은 메이저 안의 최신으로 옮기려면 `uvx --refresh --with "python-hwpx[preview]>=6.4.0,<7" --from "python-hwpx-automation[mcp,oracle]>=7.1.0,<8" hwpx-automation-mcp --help`를 한 번 실행합니다. 자동 갱신은 번들 런처 `scripts/hwpx-automation-mcp`(Claude Code·Codex 번들) 경로만 제공합니다.

...
| 🔌 | [`hwpx-mcp-server`](https://github.com/airmang/hwpx-mcp-server) | MCP 클라이언트에서 HWPX 조작 |
...
| 구분 | 의미 | 현재 값 |
|---|---|---|
| 공개 릴리스 | 현재 공개된 구성요소 버전 | `python-hwpx 3.8.0` · `hwpx-mcp-server 4.4.1` · `hwpx-plugin 0.6.8` |
| 최소 호환 버전 | 이 공개 스킬 계약이 지원하는 가장 낮은 조합 | `python-hwpx >= 3.3.1` · `hwpx-mcp-server >= 4.3.0` · skill `>= 0.5.0` |
| 플러그인 설치 핀 | 재현 가능한 설치를 위해 고정한 정확 버전 | `python-hwpx[visual,preview]==3.8.0` · `hwpx-mcp-server==4.4.1` |
...
- **문서 능력 한 벌** — 읽기·양식 채움·생성·편집·공문서·신구대조표·mail merge를 자연어 요청만으로 (엔진은 python-hwpx)
...
[hwpx-mcp-server](https://github.com/airmang/hwpx-mcp-server)가 수행합니다. 산출물이 실제

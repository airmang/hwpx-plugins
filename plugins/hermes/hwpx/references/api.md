...
3. a plugin-local venv populated with `hwpx-mcp-server==2.3.1` on first MCP start
4. `uvx --from hwpx-mcp-server==2.3.1 hwpx-mcp-server` fallback when `uv` is unavailable

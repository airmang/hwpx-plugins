...
| 미발행 후보 | 이 checkout에서 검증 중이며 아직 공개 릴리스가 아닌 조합 | `python-hwpx 5.0.2` · `python-hwpx-automation 6.0.4` · `hwpx-plugin 1.0.1` |
| 최소 호환 버전 | 1.0 스킬 계약이 지원하는 가장 낮은 조합 | `python-hwpx >= 5.0.0` · `python-hwpx-automation >= 6.0.0` · skill `>= 1.0.0` |
| 플러그인 설치 핀 | 후보 검증용 번들이 고정한 정확 버전 | `python-hwpx[preview]==5.0.2` · `python-hwpx-automation[mcp,oracle]==6.0.4` |

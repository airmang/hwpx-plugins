...
3. a plugin-local venv populated with `hwpx-mcp-server==2.3.2` on first MCP start
4. `uvx --from hwpx-mcp-server==2.3.2 hwpx-mcp-server` fallback when `uv` is unavailable

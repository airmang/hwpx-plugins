...
처음 보는 양식(예시·안내문·지워야 할 내용이 섞인 실물)은 경로 선택 전에 **정찰부터** 한다:

1. `scan_form_guidance(path)` — 비변형 정찰. 양식 스스로 선언한 색 범례(검정 유지/파랑
   수정/빨강 삭제 류)를 파싱하고 지울 것·수정 대상·placeholder(◯◯◯/\*\*/□□□)·조건부
   선택 블록·빈 셀(인접 라벨+서식 컨텍스트)·질문 목록을 table_patch 좌표로 보고한다.
   `markdownReport`를 사용자에게 그대로 보여줘라. (도구가 없으면 `hwpx-mcp-server`
   toolSurface에서 `scan_form_guidance` 존재를 확인하라 — 버전 숫자 추정 금지.)
2. **상의**: 질문 목록(빈칸 값·조건부 선택·범례 밖 색)을 사용자와 해소하고, "어디에 뭘
   넣고/지우고/둘지" fill-plan을 합의한다. **사용자 승인 없이 delete_table/delete_column
   등 파괴적 op를 실행하지 않는다.**
3. 합의 후 아래 결정표로 실행 경로(①~④)를 고른다. 조건부 선택 블록("2개 중 하나만
   남기고 삭제")을 지울 때는 **딸린 헤딩·캡션·설명 문단도 함께** 지워야 고아 헤딩이
   안 남는다(2026-07-06 평가계획 산출물 실패 사례).
4. **제출 확언 금지 규칙**: 스코어/verify 통과는 필요조건일 뿐이다. "제출 가능"은

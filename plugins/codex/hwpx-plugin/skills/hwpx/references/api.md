...
| 최소 호환 버전 | 이 공개 스킬 계약이 지원하는 가장 낮은 조합 | core `>=3.3.1` · MCP `>=4.3.0` · skill `>=0.6.8` |
| 플러그인 설치 핀 | 공개 번들이 재현 가능한 설치에 사용하는 정확 버전 | `python-hwpx[visual,preview]==3.8.0` · `hwpx-mcp-server==4.4.1` |
...
S-083 공개 릴리스 `hwpx-mcp-server 4.4.1`의 트랜잭션·서식·그림·비교·생성기·문서 지도 도구 요약이다.
...
3. an immutable plugin-local runtime fingerprinted by the exact
   `hwpx-mcp-server==4.4.1`, `python-hwpx[visual,preview]==3.8.0`, skill, Python ABI,
   and platform values

...
| 릴리스 상태 | 이 checkout의 train 상태 — `unreleased-candidate`: 아직 공개되지 않은 미발행 후보 | `python-hwpx 5.6.0` · `python-hwpx-automation 6.6.4` · `hwpx-plugin 1.6.0` |
| 최소 호환 버전 | 1.6 스킬 계약이 지원하는 가장 낮은 조합 | core `>=5.6.0` · automation `>=6.5.0` · skill `>=1.6.0` |
| 플러그인 설치 핀 | 번들이 재현 검증에 사용하는 정확 버전 | `python-hwpx[preview]==5.6.0` · `python-hwpx-automation[mcp,oracle]==6.6.4` |
...
6.6.4 후보 `python-hwpx-automation`의 트랜잭션·서식·그림·비교·생성기·문서 지도 도구 요약이다.

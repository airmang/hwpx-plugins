...
| 구분 | 의미 | 현재 값 |
|---|---|---|
| 공개 릴리스 | 현재 공개된 S-083 구성요소 버전 | `python-hwpx 3.4.1` · `hwpx-mcp-server 4.3.0` · `hwpx-plugin 0.6.1` |
| 최소 호환 버전 | 이 공개 스킬 계약이 지원하는 가장 낮은 조합 | `python-hwpx >= 3.3.1` · `hwpx-mcp-server >= 4.3.0` · skill `>= 0.6.1` |
| 플러그인 설치 핀 | 공개 번들이 재현 가능한 설치를 위해 고정한 정확 버전 | `python-hwpx[visual]==3.4.1` · `hwpx-mcp-server==4.3.0` |

...
| 릴리스 상태 | 이 checkout의 train 상태 — `released`: 3스택 원격 정본과 실제 marketplace 설치까지 관찰 완료 | `python-hwpx 5.2.0` · `python-hwpx-automation 6.2.1` · `hwpx-plugin 1.2.0` |
| 최소 호환 버전 | 1.2 스킬 계약이 지원하는 가장 낮은 조합 | core `>=5.2.0` · automation `>=6.2.0` · skill `>=1.2.0` |
| 플러그인 설치 핀 | 번들이 재현 검증에 사용하는 정확 버전 | `python-hwpx[preview]==5.2.0` · `python-hwpx-automation[mcp,oracle]==6.2.1` |
...
공개 릴리스 `python-hwpx-automation 6.2.1`의 트랜잭션·서식·그림·비교·생성기·문서 지도 도구 요약이다.

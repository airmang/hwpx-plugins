...
| 릴리스 상태 | 이 checkout의 train 상태 — `release-approved`: 발행 승인·진행 중, 원격 truth 관찰 전 | `python-hwpx 5.0.1` · `python-hwpx-automation 6.0.0` · `hwpx-plugin 1.0.0` |
| 최소 호환 버전 | 1.0 후보 스킬 계약이 지원할 예정인 가장 낮은 조합 | core `>=5.0.0` · automation `>=6.0.0` · skill `>=1.0.0` |
| 플러그인 설치 핀 | 후보 번들이 재현 검증에 사용하는 정확 버전 | `python-hwpx[preview]==5.0.1` · `python-hwpx-automation[mcp,oracle]==6.0.0` |

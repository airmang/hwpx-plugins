#!/usr/bin/env python3
"""Create an operating-plan HWPX and inspect its quality profile."""
                ...
                "rows": [
                    {
                        "phase": "준비",
                        "period": "3월",
                        "activity": "운영 계획 공유 및 협의회 구성",
                        "owner": "운영팀",
                    },
                    {
                        "phase": "운영",
                        "period": "4월~11월",
                        "activity": "AI 활용 수업, 프로젝트, 교원 연수 운영",
                        "owner": "교과협의회",
                    },
                    {
                        "phase": "평가",
                        "period": "12월~2월",
                        "activity": "성과 분석, 사례 공유, 차년도 개선안 수립",
                        "owner": "평가팀",
                    },
                ],
            },
...
def main() -> int:
    ...
    profile = inspect_operating_plan_quality(OUTPUT, plan=plan)
    print(f"output={OUTPUT}")
    print(f"quality_pass={report['pass']}")
    print(f"operating_plan_pass={profile['pass']}")
    print(f"operating_plan_score={profile['score']}")
    print(f"visual_review_required={profile['visual_review_required']}")
    if not report["pass"] or not profile["pass"]:

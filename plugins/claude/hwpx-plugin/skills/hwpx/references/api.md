...
| 릴리스 상태 | 이 checkout의 train 상태 — `unreleased-candidate`: 누름틀 필드 저작(`add_form_field`) 1.1.0 미발행 후보 | `python-hwpx 5.1.1` · `python-hwpx-automation 6.1.2` · `hwpx-plugin 1.1.0` |
| 최소 호환 버전 | 1.1 스킬 계약이 지원하는 가장 낮은 조합 | core `>=5.1.0` · automation `>=6.1.0` · skill `>=1.1.0` |
| 플러그인 설치 핀 | 번들이 재현 검증에 사용하는 정확 버전 | `python-hwpx[preview]==5.1.1` · `python-hwpx-automation[mcp,oracle]==6.1.2` |

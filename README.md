...
> [!NOTE]
> 이 소스 트리는 아직 공개되지 않은 1.1.0 미발행 후보입니다(누름틀 필드 저작
> `add_form_field` 트레인). 현재 공개 트레인은 `python-hwpx 5.0.2` ·
> `python-hwpx-automation 6.0.4` · `hwpx-plugin 1.0.1`입니다.
...
| 이 checkout | 아직 공개되지 않은 1.1.0 후보 — 누름틀 필드 저작(`add_form_field`) 트레인 | `python-hwpx 5.1.1` · `python-hwpx-automation 6.1.2` · `hwpx-plugin 1.1.0` |
| 최소 호환 버전 | 1.1 스킬 계약이 지원하는 가장 낮은 조합 | `python-hwpx >= 5.1.0` · `python-hwpx-automation >= 6.1.0` · skill `>= 1.1.0` |
| 플러그인 설치 핀 | 번들이 고정한 정확 버전 | `python-hwpx[preview]==5.1.1` · `python-hwpx-automation[mcp,oracle]==6.1.2` |

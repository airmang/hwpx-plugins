...
| 용어 | 의미 | 현재 값 |
|---|---|---|
| 완전한 공개 트레인 | 마지막으로 plugin 설치까지 함께 검증한 조합 (released 2026-07-31, 네이티브 차트 생성 `add_chart` 트레인) | `python-hwpx 5.3.0` · `python-hwpx-automation 6.3.1` · `hwpx-plugin 1.3.0` |
| 릴리스 상태 | 이 checkout의 train 상태 — `unreleased-candidate`: 미발행 후보 (표 테두리·음영 `format_table` 확장·수리된 core 기본값 트레인) | `python-hwpx 5.4.0` · `python-hwpx-automation 6.4.0` · `hwpx-plugin 1.4.0` |
| 최소 호환 버전 | 1.4 스킬 계약이 지원하는 가장 낮은 조합 | core `>=5.4.0` · automation `>=6.4.0` · skill `>=1.4.0` |
| 플러그인 설치 핀 | 번들이 재현 검증에 사용하는 정확 버전 | `python-hwpx[preview]==5.4.0` · `python-hwpx-automation[mcp,oracle]==6.4.0` |
...
이 스킬의 최종 HWPX 산출물 작성에는 최소 호환 버전인 `python-hwpx >= 5.4.0`이 필요하다.
...
6.4.0 후보 `python-hwpx-automation`의 트랜잭션·서식·그림·비교·생성기·문서 지도 도구 요약이다.

...
def test_api_reference_requires_current_open_safety_stack() -> None:
    ...
    for reference in references:
        ...
        assert "`python-hwpx 3.2.0`" in text
...
def test_product_identity_is_the_name_version_and_maturity_authority() -> None:
    ...
    assert components["core"]["currentVersion"] == "3.2.0"
    assert components["core"]["minimumCompatibleVersion"] == "3.2.0"
    assert components["mcp"]["currentVersion"] == "4.1.0"
    assert components["mcp"]["minimumCompatibleVersion"] == "4.1.0"
    assert components["plugin"]["currentVersion"] == "0.4.0"
    assert components["plugin"]["minimumCompatibleVersion"] == "0.4.0"
